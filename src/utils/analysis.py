...
class Security:
    ...
    def getRiskComp(self, riskFree=.02, riskAversion=1):
...
class Portfolio(Security):
    def __init__(self, name, securities):
        ...
        self.initWeights()
        super().__init__(self)
        self.returnFrame=self.mergeFrames("Returns")
        self.priceFrame=self.mergeFrames("Close_Price")
        self.wealthFrame=self.mergeFrames("T_Wealth")
        self.mergeStats()
        self.getDashStats()
    ...
    def getPrices(self):
        ...
        self.securityDF["Close_Price"]=prices
        #copy this dataframe and rename it so that we have all the prices of all securities and portoflio in 1 frame
        self.priceDF=self.securityDF.copy()
        newnames=dict(zip([n for n in self.priceDF],secNames))
        self.priceDF.rename(newnames,axis=1,inplace=True)
        #select the subset of only the closing prioce of the portfolio.
        self.securityDF=self.securityDF[["Close_Price"]].copy()
        return(self.securityDF)

    def mergeFrames(self,colName):
        secNames=["Date","Portfolio"]+[n.name for n in self.securities]
        mFrame=self.securityDF.loc[:,[colName]].copy()
        for security in self.securities:
            #Have to get rid of tuple if there is only one security.
            if len(secNames)>1:
                mFrame = mFrame.merge(security.securityDF.loc[:,["Date",colName]], on='Date')
                newnames=dict(zip([n for n in mFrame],secNames))
                mFrame.rename(newnames,axis=1,inplace=True)
            else:
                mFrame = mFrame.merge(security.securityDF.loc[:,["Date",colName]], on='Date')
        return(mFrame)
